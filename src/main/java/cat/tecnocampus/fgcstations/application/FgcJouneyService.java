...
public class FgcJouneyService {
    ...
    public Journey getJourneyDomain(String origin, String destination) {
        ...
        //  try no to use any sql (jpql) query, just come up with an appropriate method name
            return journeyRepository.findByOriginNameAndDestinationName(origin, destination).orElseThrow(() ->
                    new JourneyDoesNotExistsException( origin, destination)
            );
    }
    ...
    public JourneyId getJourneyId(String origin, String destination) {
        ...
        //  try no to use any sql (jpql) query, just come up with an appropriate method name
       /* Optional<Journey> journey = journeyRepository.findByOriginNameAndDestinationName(origin, destination);
        return journey.orElseThrow(() ->
                new JourneyDoesNotExistsException("Journey from " + origin + " to " + destination + " does not exist")
        );
        journey.get*/
        return null;
    }
