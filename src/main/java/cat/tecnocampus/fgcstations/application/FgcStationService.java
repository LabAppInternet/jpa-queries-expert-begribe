...
public class FgcStationService {
    ...
    public Station getStation(String name) {
        ...
        Optional<Station> station = stationRepository.findById(name);
        if(station.isEmpty()){
            throw new StationDoesNotExistsException("Empty");
        }
        return station.get();
    }
    ...
    public StationDTO getStationDTO(String name) {
        ...
        /*Optional<StationDTO> station = stationRepository.findByIdDTO(name);
        if(station.isEmpty()){
            throw new StationDoesNotExistsException("Empty").getMessage();
        }*/
        return stationRepository.findByIdDTO(name).orElseThrow(() -> new StationDoesNotExistsException(name));
